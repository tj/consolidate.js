...
exports.expressVue = fromStringRenderer('expressVue');

/**
 * HTMLing string support.
 */

exports.expressVue.render = function(str, options, fn) {
    console.log(options.settings);
    if (!options.settings.vue) {
        options.settings.vue = {
            layoutsDir: options.settings.views + '/',
            componentsDir: options.settings.views + '/',
            defaultLayout: 'layout'
        };
    }
console.log(options.settings.vue);
  return promisify(fn, function (fn) {
    var engine = requires.expressVue || (requires.expressVue = require('express-vue'));
    try {
      var tmpl = cache(options) || cache(options, engine);
      tmpl(options.filename, options, fn);
    } catch (err) {
      fn(err);
    }
  });
