...
function requireReact(module, filename) {
  ...
  var compiled = babel.transform(content, { presets: ['es2015', 'react'] }).code;
...
function requireReactString(src, filename) {
  ...
  // Compile Using React
  src = babel.transform(src, { presets: ['es2015', 'react'] }).code;
...
function reactRenderer(type){
  ...
  return function(str, options, fn) {
    return promisify(fn, function(fn) {
      ...
      var reactDOMServer = requires.reactDOMServer || (requires.reactDOMServer = require('react-dom/server'));
