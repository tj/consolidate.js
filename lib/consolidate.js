...
exports.minstache = fromStringRenderer('minstache');

/**
 * minstache string support.
 */

exports.minstache.render = function(str, options, fn){
  var engine = requires.minstache || (requires.minstache = require('minstache'));
  try {
    var minstache = cache(options) || cache(options, engine(str));
    fn(null, minstache(options));
  } catch (err) {
    fn(err);
  }
};
