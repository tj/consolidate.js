...
/**
 * Contemplate support. https://github.com/foo123/Contemplate
 */

exports.contemplate = fromStringRenderer('Contemplate');

/**
 * Contemplate custom engine load.
 */

exports.contemplate.load = function( Contemplate, options ) {
    if ( Contemplate ) 
    {
        requires.Contemplate = Contemplate;
        // in memory only caching used here
        Contemplate.setCacheMode( Contemplate.CACHE_TO_DISK_NONE );
    }
};

/**
 * Contemplate string support.
 */

exports.contemplate.render = function(str, options, fn) {
  var engine = requires.Contemplate || (requires.Contemplate = require('Contemplate'));
  try {
    for (var partial in options.partials) {
        engine.addInline( partial, options.partials[partial] );
    }
    var tplID = options.filename;
    engine.addInline( tplID, str );
    var tmpl = cache(options) || cache(options, engine.tpl(tplID, null, options));
