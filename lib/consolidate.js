...
/**
 * Vue support.
 * using https://github.com/express-vue/vue-pronto
 * however, it doesn't support rendering a string template, only "files"
 * so i'm using its vue, vueify and vue-server-renderer dependencies to do that
 */

/**
 *  Require from string
 *  https://stackoverflow.com/a/17585470/493756
 *  or we could use https://www.npmjs.com/package/require-from-string
 */
...
function vueRenderer(type) {
  var Pronto = requires.pronto || (requires.pronto = require('vue-pronto/lib/index'));
  var prontoRendererCaches = {lruCache: null, internalCache: null};

  var vue = requires.vue || (requires.vue = require('vue'));
  var vueify = requires.vueify || (requires.vueify = require('vueify'));
  var vueServerRenderer = requires.vueServerRenderer || (requires.vueServerRenderer = require('vue-server-renderer'));
  var ssRenderer = vueServerRenderer.createRenderer();
  ...
  return function(str, options, cb) {
    return promisify(cb, function(cb) {
      ...
      if (type === 'path') {
        ...
        var fullPath = resolve(str);
        var rootPath = process.cwd();
        var filepath = fullPath.replace(rootPath + '/', '');
        var prontoOptions = {rootPath: rootPath, vueVersion: { disabled: true }};
        if (!options.cache) {
          // disable pronto lru cache
          prontoOptions.cacheOptions = {max: 0};
        }
        var prontoRenderer = new Pronto(prontoOptions);
        ...
        if (options.cache) {
          prontoRenderer.lruCache = prontoRendererCaches.lruCache || prontoRenderer.lruCache;
          prontoRenderer.internalCache = prontoRendererCaches.internalCache || prontoRenderer.lruCache;
          prontoRendererCaches.lruCache = prontoRenderer.lruCache;
          prontoRendererCaches.internalCache = prontoRenderer.internalCache;
        } else {
          // disable pronto internal cache
          // yup
          prontoRenderer.internalCache = {get: function() {}, set: function() {}};
        }

        // prontoRenderer its own cache in it
        prontoRenderer.RenderToString(filepath,
          {},
          {
            propsData: options
          })
        ...
        // make up a fake path
        fullPath = path.join(__dirname, '/' + hashCode(str));

        var Component;
        if (options.cache) {
          Component = cache({filename: fullPath, cache: fullPath});
        }

        var onComponent = function(err, Component) {
          if (err) {
            return cb(err);
          }
          if (options.cache) {
            cache({filename: fullPath, cache: fullPath}, Component);
          }

          var Factory = vue.extend(Component);
          var instance = new Factory({ propsData: options });
          ssRenderer.renderToString(instance, function(err, html) {
            if (err) {
              return cb(err);
            }
            cb(err, html);
        ...
        if (Component) {
          return onComponent(null, Component);
        }

        if (/<template/.test(str.trim())) {
          // assume that the str is a fileContent of a .vue file
          vueify.compiler.compile(str, fullPath, function(err, js) {
            onComponent(err, requireFromString(js, fullPath));
          });
        } else {
          // assume that the str the content of a <template> ... </template>
          onComponent(null, { template: str, data: function() { return options; }});
        }
      }
    });
  };
 ...
 * vue path support.
 */
exports.vue = vueRenderer('path');

/**
 * vue string support.
 */
exports.vue.render = vueRenderer('string');
