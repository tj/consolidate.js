...
function read(path, options, fn) {
  ...
  // read
  if (nowReading[path]) {
    if (!readListeners[path]) readListeners[path] = [fn];
    else readListeners[path].push(fn);
  }else{
    nowReading[path] = true;
    fs.readFile(path, 'utf8', function(err, str){
      if (err) return fn(err);
      // remove extraneous utf8 BOM marker
      str = str.replace(/^\uFEFF/, '');
      nowReading[path] = false;
      if (options.cache) readCache[path] = str;
      if (readListeners[path]) {
        readListeners[path].forEach(function(fn) { fn(null, str) });
        readListeners[path] = null;
      }
      fn(null, str);
    });
  }
